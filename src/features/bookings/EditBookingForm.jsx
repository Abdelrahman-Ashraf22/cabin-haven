...
function EditBookingForm({ onCloseModal, bookingToEdit = {} }) {
  // destructuring these values to the bookingToEdit object
  const { id, ...editValues } = bookingToEdit;

  // destructuring this value to the editValues
  const { created_at } = editValues;

  // Getting these functions from useUpdateBooking custom hook
  ...
  // Getting these functions from the useForm => React hook form
  const {
    register,
    handleSubmit,
    watch,
  ...
  // Getting these value from useDatePicker custom hook
  const { range, setRange, handleDayClick } = useDatePicker();

  // Geting the booking dates from useGetBookingsByCabin custom hook
  ...
  // Getting the cabin through its ID
  ...
  // The booked date for each cabin "the starting & ending dates"
